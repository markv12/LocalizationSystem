...
public static class UIToolkitLocalizer {
    ...
    public static void ApplyFont(VisualElement element, FontLocalizer.Type type) {
        ...
        TextCoreFontAsset font = Localizer.CurrentLangUIToolkitFont(type);
        if (font == null) {
    ...
    public static void ApplyFonts(VisualElement root) {
        if (root == null) return;
        TextCoreFontAsset regular = Localizer.CurrentLangUIToolkitFont(FontLocalizer.Type.Regular);
        TextCoreFontAsset title = Localizer.CurrentLangUIToolkitFont(FontLocalizer.Type.Title);
        if (regular == null && title == null) {
            ApplyFont(root, FontLocalizer.Type.Regular);
            return;
        }

        StyleFontDefinition regularStyle = regular != null ? FontDefinition.FromSDFFont(regular) : default;
        StyleFontDefinition titleStyle = title != null ? FontDefinition.FromSDFFont(title) : regularStyle;
        if (regular == null) regularStyle = titleStyle;

        Apply(root);

        void Apply(VisualElement element) {
            bool isTitle = element.ClassListContains(TITLE_FONT_CLASS);
            element.style.unityFontDefinition = isTitle ? titleStyle : regularStyle;
